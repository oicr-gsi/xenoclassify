...
while(my $mouserec=<$MOUSE>){
	...
	# Therefore, the following expression will evaluate to true as long as 0X4 is true.
	# Any reads with flag 4 will be marked as unmapped or "0".
	# In this script, a read is mapped if the flag is not present.
	if ($mfields[1] & 4)
	{
		$mmapped=0;
	}
