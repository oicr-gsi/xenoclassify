...
def initialize_user_input(args):
	...
	mouse_bam = pysam.AlignmentFile(args.mouse,"rb")
	human_bam = pysam.AlignmentFile(args.human,"rb")
	output_dir = args.output
	return mouse_bam, human_bam, output_dir

def create_fastq_output(bam):
	# create output files
	# FASTQs
	graft = pysam.AlignmentFile("graft.fastq", "wb", template = bam)
	host = pysam.AlignmentFile("host.fastq", "wb", template = bam)
	both = pysam.AlignmentFile("both.fastq", "wb", template = bam)
	ambiguous = pysam.AlignmentFile("ambiguous.fastq", "wb", template = bam)
	neither = pysam.AlignmentFile("neither.fastq", "wb", template = bam)
...
def create_bam_output(bam):
	# BAMs
	out_bam = pysam.AlignmentFile("output.bam", "wb", template = bam)
	return out_bam
...
def filter_bam(bam, species):
	primary = pysam.AlignmentFile("{:s}_primary.bam".format(species), "wb", template=bam)
	secondary = pysam.AlignmentFile("{:s}_secondary.bam".format(species), "wb", template=bam)
...
def increment_counters(classification):
...
if __name__ == '__main__':
	...
	args = parse_input()
	mouse_bam, human_bam, output_dir = initialize_user_input(args)
	...
	for mouse_read, human_read in zip_longest(mouse_primary.fetch(until_eof=True), human_primary.fetch(until_eof=True)):
		...
		if read_count % 2 == 0:
			...
			classification = classify(mr1_AS, mr2_AS, hr1_AS, hr2_AS)
 			counters = increment_counters(classification)
